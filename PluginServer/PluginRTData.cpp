...
void CPluginRTData::SetQuoteReqData(int nCmdID, const Json::Value &jsnVal, SOCKET sock)
{
	...
	if ( bNeedSub )
	{
		StockSubErrCode err_code = m_pQuoteServer->SubscribeQuote(req.body.strStockCode, (StockMktType)req.body.nStockMarket, QUOTE_SERVER_TYPE, true);
		...
		if ( m_pQuoteData->IsRTDataExist(nStockID) )
		{
			...
			if ( m_pQuoteData->FillRTData(nStockID, pQuoteRT, nCount) )
			{
				...
				int nLastTrueNum = nCount;
				for ( int i = nCount; i > 0; i--)
				{
					if ( ack.vtRTData[i - 1].nDataStatus == 1 )
					{
						break;
					}
					nLastTrueNum--;
				}
				ack.nLastTrueDataNum = nLastTrueNum;
			}
