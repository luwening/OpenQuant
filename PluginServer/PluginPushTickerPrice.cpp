...
void CPluginPushTickerPrice::SetQuoteReqData(int nCmdID, const Json::Value &jsnVal, SOCKET sock)
{
...
void CPluginPushTickerPrice::NotifyQuoteDataUpdate(int nCmdID, INT64 nStockID)
{
...
void CPluginPushTickerPrice::PushStockData(INT64 ddwStockHash, SOCKET sock, INT64 nSequence)
{
	std::vector<PluginTickItem> vtTickPrice;
	vtTickPrice.resize(200);
	int nTickFillCount = m_pQuoteData->FillTickArr(ddwStockHash, _vect2Ptr(vtTickPrice), _vectIntSize(vtTickPrice));
	if ( nTickFillCount >= 0 )
	{
		...
		m_pQuoteData->GetStockInfoByHashVal(ddwStockHash, eMkt, szStockCode);
		...
		for ( int n = 0; n < nValidNum; n++ )
		{
			...
			tickItem.nDealType = srcItem.nDealType;
			tickItem.nSequence = srcItem.nSequence;
			if ( tickItem.nSequence <= nSequence )
			{
				continue;
			}
			tickItem.nVolume = srcItem.nVolume;
...
void CPluginPushTickerPrice::OnTimeEvent(UINT nEventID)
{
...
void CPluginPushTickerPrice::OnMsgEvent(int nEvent,WPARAM wParam,LPARAM lParam)
{
...
void CPluginPushTickerPrice::ClearQuoteDataCache()
{
...
void CPluginPushTickerPrice::HandleTimeoutReq()
{
...
void CPluginPushTickerPrice::ReplyAllReadyReq()
{
	DWORD dwTickNow = ::GetTickCount();
	MAP_STOCK_DATA_REQ::iterator it_stock = m_mapReqInfo.begin();
	for ( ; it_stock != m_mapReqInfo.end(); )
...
void CPluginPushTickerPrice::ReplyStockDataReq(StockDataReq *pReq, const QuoteAckDataBody &data)
{
...
void CPluginPushTickerPrice::ReplyDataReqError(StockDataReq *pReq, int nErrCode, LPCWSTR pErrDesc)
{
	CHECK_RET(pReq && m_pQuoteServer, NORET);
...
void CPluginPushTickerPrice::SetTimerHandleTimeout(bool bStartOrStop)
{
...
void CPluginPushTickerPrice::SetTimerClearCache(bool bStartOrStop)
{
...
bool CPluginPushTickerPrice::GetStockMktCode(INT64 nStockID, StockMktCode &stkMktCode)
{
...
void CPluginPushTickerPrice::ClearAllReqCache()
{
	MAP_STOCK_DATA_REQ::iterator it_stock = m_mapReqInfo.begin();
