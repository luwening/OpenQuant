#include "stdafx.h"
#include "PluginCheckSecNum.h"
#include "PluginHKTradeServer.h"
#include "Protocol/ProtoCheckSecNum.h"
...
//tomodify 2
#define PROTO_ID_QUOTE		PROTO_ID_TDHK_CHECK_SECNUM
typedef CProtoCheckSecNum	CProtoQuote;

//////////////////////////////////////////////////////////////////////////

CPluginCheckSecNum::CPluginCheckSecNum()
{	
	m_pTradeOp = NULL;
	m_pTradeServer = NULL;
...
void CPluginCheckSecNum::Init(CPluginHKTradeServer* pTradeServer, ITrade_HK*  pTradeOp)
{
...
void CPluginCheckSecNum::Uninit()
{
	if ( m_pTradeServer != NULL )
	{
		m_pTradeServer = NULL;
...
void CPluginCheckSecNum::SetTradeReqData(int nCmdID, const Json::Value &jsnVal, SOCKET sock)
{
	...
	//tomodify 3
	CheckSecNumReqBody &body = req.body;
	...
	if ( !bRet )
	{
		...
		CA::Unicode2UTF(L"验证失败", ack.head.strErrDesc);
	...
	bool bIsCheck = false;
	//IManage_SecurityNum::GetIsCheck(bIsCheck);
	//if ( !bIsCheck )
	//{
	TradeAckType ack;
	ack.head = req.head;
...
bool CPluginCheckSecNum::DoDeleteReqData(StockDataReq* pReq)
{
...
void CPluginCheckSecNum::NotifyOnUnlockTrade(UINT32 nCookie, Trade_SvrResult enSvrRet, UINT64 nErrCode)
{
...
void CPluginCheckSecNum::OnTimeEvent(UINT nEventID)
{
...
void CPluginCheckSecNum::OnMsgEvent(int nEvent,WPARAM wParam,LPARAM lParam)
{
...
void CPluginCheckSecNum::HandleTimeoutReq()
{
...
void CPluginCheckSecNum::HandleTradeAck(TradeAckType *pAck, SOCKET sock)
{
...
void CPluginCheckSecNum::SetTimerHandleTimeout(bool bStartOrStop)
{
...
void CPluginCheckSecNum::ClearAllReqAckData()
{
	VT_REQ_TRADE_DATA::iterator it_req = m_vtReqData.begin();
	for ( ; it_req != m_vtReqData.end(); )
	{
