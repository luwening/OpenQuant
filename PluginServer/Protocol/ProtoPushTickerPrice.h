...
class CProtoPushTickerPrice: public CProtoParseBase
{
public:
	//tomodify 1 定义协议请求、协议响应结构体类型
	typedef PushTicker_Req		ProtoReqDataType;
	typedef PushTicker_Ack		ProtoAckDataType;

	//tomodify 2 定义协议请求、协议响应包体类型
	typedef PushTickerReqBody	ProtoReqBodyType;
	typedef PushTickerAckBody	ProtoAckBodyType;


public:
	CProtoPushTickerPrice();
	virtual ~CProtoPushTickerPrice();
