...
class CProtoExRightInfo: public CProtoParseBase
{
public:
	//tomodify 1 定义协议请求、协议响应结构体类型
	typedef ExRightInfo_Req		ProtoReqDataType;
	typedef ExRightInfo_Ack		ProtoAckDataType;

	//tomodify 2 定义协议请求、协议响应包体类型
	typedef ExRightInfoReqBody		ProtoReqBodyType;
	typedef ExRightInfoAckBody		ProtoAckBodyType;


public:
	CProtoExRightInfo();
	virtual ~CProtoExRightInfo();
