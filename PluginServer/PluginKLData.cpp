...
void CPluginKLData::SetQuoteReqData(int nCmdID, const Json::Value &jsnVal, SOCKET sock)
{
	...
	if ( bNeedSub )
	{
		StockSubErrCode err_code = m_pQuoteServer->SubscribeQuote(req.body.strStockCode, (StockMktType)req.body.nStockMarket, QUOTE_SERVER_TYPE, true, req.body.nKLType);
		if ( err_code != StockSub_Suc )
		{
			ReplyDataReqError(&(*pReqInfo), PROTO_ERR_UNKNOWN_ERROR, L"订阅数据失败！");
...
void CPluginKLData::NotifyQuoteDataUpdate(int nCmdID, INT64 nStockID, int nKLType)
{
	...
	//MAP_STOCK_DATA_REQ::iterator it_stock = m_mapReqInfo.find(std::make_pair<INT64, int>(nStockID, nKLType));
	//if  ( it_stock != m_mapReqInfo.end() )
	//{
	//	VT_STOCK_DATA_REQ vtReq = it_stock->second;
	//	nRehabType = it_stock->second
	//}
